...
class UsbCamPublisher(Node):
    def __init__(self):
        ...
        self.switch_interval = 2.0  # seconds
...
# 전환 직후 → 검은 화면 1회 송출
        ...
        self.get_logger().info(
            f'Published one frame ({len(data)} bytes)'
        )

    def _switch_camera(self):
        #0 → 1 → 0 순서로 카메라 전환
        self.current_cam_idx = 1 - self.current_cam_idx
        self.get_logger().info(
            f"Switched camera! Now: {self.cameras[self.current_cam_idx]}"
        )
        self.send_black_frame = True

    def _publish_black_frame(self):
    #검은 화면 1장 publish (JPEG 파일 미리 준비)
        try:
            black_frame_path = Path('/tmp/black_frame.jpg')  # 미리 생성해둔 검은 화면 JPEG
            if not black_frame_path.exists():
                self.get_logger().error("검은 화면 파일이 없습니다! (/tmp/black_frame.jpg)")
                return

            data = black_frame_path.read_bytes()
            msg = CompressedImage()
            msg.header.stamp = self.get_clock().now().to_msg()
            msg.format = 'jpeg'
            msg.data = data
            self.publisher_.publish(msg)
            self.get_logger().info("Published BLACK frame (transition marker)")

        except Exception as e:
            self.get_logger().error(f"Failed to publish black frame: {e}")
